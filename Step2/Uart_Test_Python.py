...
def Open_Port():
    ...
    open_port.config(bd = 0 , fg = 'gray')

    close_port.config(bd = 3 , fg = 'black')
    ...
    ser = serial.Serial(USB_port.get(),int(BaudRate.get()),bytesize = int(Data_Bit.get()),parity = Parity_Bit , stopbits = STOP_BITS)
...
def Close_Port():
    ...
    open_port.config(bd = 3 , fg = 'black')

    close_port.config(bd = 0 , fg = 'gray')
...
if __name__ =='__main__':
    ...
    USB_port = ttk.Combobox(frame1,values = ["/dev/ttyUSB0","/dev/ttyUSB1","/dev/ttyUSB0","/dev/ttyUSB3","/dev/ttyUSB4"])
    ...
    close_port = Button(frame1,text = "close port",width = 8,command = Close_Port,bd = 0 , fg = 'gray')
